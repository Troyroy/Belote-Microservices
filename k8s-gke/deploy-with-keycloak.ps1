...
Write-Host "WARNING: Make sure you have updated 02-secrets-aiven.yaml with your Aiven credentials!" -ForegroundColor Yellow
...
Write-Host "  Password: admin_secure_password" -ForegroundColor Yellow
Write-Host ""
...
if ($response -ne 'y' -and $response -ne 'Y') {
    ...
    Write-Host "  3. Login: admin / admin_secure_password" -ForegroundColor Yellow
...
$monitoringResponse = Read-Host "Do you want to deploy Prometheus and Grafana? (y/n)"
if ($monitoringResponse -eq 'y' -or $monitoringResponse -eq 'Y') {
...
if ($usersPod) {
    ...
    if ($usersErrors) {
        Write-Host "WARNING: Users microservice has some errors. Check logs:" -ForegroundColor Yellow
        Write-Host "  kubectl logs $usersPod -n microservices" -ForegroundColor Cyan
    } else {
        Write-Host "PASS: Users microservice logs look good!" -ForegroundColor Green
    }
}
...
if ($gamesPod) {
    ...
    if ($gamesErrors) {
        Write-Host "WARNING: Games microservice has some errors. Check logs:" -ForegroundColor Yellow
        Write-Host "  kubectl logs $gamesPod -n microservices" -ForegroundColor Cyan
    } else {
        Write-Host "PASS: Games microservice logs look good!" -ForegroundColor Green
    }
}
Write-Host ""
...
if ($GATEWAY_IP) {
    Write-Host "SUCCESS: API Gateway is accessible at: http://$GATEWAY_IP:8080" -ForegroundColor Green
    Write-Host "Test with: curl http://$GATEWAY_IP:8080/actuator/health" -ForegroundColor Yellow
} else {
    Write-Host "WARNING: API Gateway IP not yet assigned. Check with: kubectl get svc api-gateway -n microservices" -ForegroundColor Yellow
}
Write-Host ""
...
if ($monitoringResponse -eq 'y' -or $monitoringResponse -eq 'Y') {
    ...
    if ($GRAFANA_IP) {
        Write-Host "SUCCESS: Grafana is accessible at: http://$GRAFANA_IP:3000" -ForegroundColor Green
        Write-Host "  Username: admin" -ForegroundColor Yellow
...
Write-Host "SUCCESS: Aiven MySQL (Games Database)" -ForegroundColor Green
Write-Host "SUCCESS: Aiven MySQL (Users Database)" -ForegroundColor Green
Write-Host "SUCCESS: Aiven MySQL (Keycloak Database)" -ForegroundColor Green
Write-Host "SUCCESS: GKE Keycloak (Authentication)" -ForegroundColor Green
Write-Host "SUCCESS: GKE Redis (In-cluster)" -ForegroundColor Green
Write-Host "SUCCESS: GKE RabbitMQ (In-cluster)" -ForegroundColor Green
Write-Host ""
